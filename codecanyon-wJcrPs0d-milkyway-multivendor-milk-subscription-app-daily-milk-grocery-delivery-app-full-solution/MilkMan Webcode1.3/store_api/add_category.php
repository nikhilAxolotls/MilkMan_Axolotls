...
if ($status == '' or $store_id == '' or $title == '') {
    $returnArr = array(
        "ResponseCode" => "401",
        "Result" => "false",
...
$table = "tbl_mcat";
            $field_values = ["title", "status","store_id","img"];
            $data_values = ["$title", "$status","$store_id","$path"];
			...
			$returnArr    = array(
                "ResponseCode" => "200",
                "Result" => "true",
                "ResponseMsg" => "Product Category Add Successfully"
            );
