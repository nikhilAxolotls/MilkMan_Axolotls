...
$h = new Medico();
	  ...
	  if($status == 1)
	  {
		   $checks = $medi->query("select uid from tbl_normal_order where id=".$oid."")->fetch_assoc(); 
		   $uid = $checks['uid'];
...
$table="tbl_notification";
  ...
  $data_values=array("$uid","$timestamp","$title_main","$description");
	  ...
	  {
		 $checks = $medi->query("select uid from tbl_order where id=".$oid."")->fetch_assoc(); 
		 $uid = $checks['uid'];
			$udata = $medi->query("select * from tbl_user where id=".$checks['uid']."")->fetch_assoc();
$name = $udata['name'];
...
$content = array(
       "en" => $name.', Your Order #'.$oid.' Has Been Rejected.'
   );
$heading = array(
   "en" => $comment_reject
);
...
$timestamp = date("Y-m-d H:i:s");
$description = $name.', Your Order #'.$oid.' Has Been Rejected.';
