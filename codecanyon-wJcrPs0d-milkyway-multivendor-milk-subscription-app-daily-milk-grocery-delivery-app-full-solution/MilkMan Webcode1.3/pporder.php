          ...
          <div class="container-fluid default-page">
		   <div class="row">
             ...
             <div class="col-sm-12">
				...
				if(isset($_GET['oid']))
				{
					$count = $medi->query("select * from tbl_subscribe_order where id=".$_GET['oid']." and store_id=".$sdata['id']."")->num_rows;
					 if($count !=0)
					 { 
					?>
					 <form class="form" method="post">
							<div class="form-body">
								...
								<div class="form-group">
									...
									<input type="hidden" name="type" value="passign_rider"/>
				...
				{
					$count = $medi->query("select * from tbl_subscribe_order where id=".$_GET['d_id']." and store_id=".$sdata['id']."")->num_rows;
					 if($count !=0)
					 { 
					?>
					 <form class="form" method="post">
							<div class="form-body">
								...
								<div class="form-group">
									...
									<input type="hidden" name="type" value="pcancle_order"/>
                ...
                <table class="display" id="basic-1">
                        <thead>
                                            ...
                                            <tbody>
											<?php 
											 $stmt = $medi->query("SELECT * FROM `tbl_subscribe_order` where status!='Cancelled' and status!='Completed' and store_id=".$sdata["id"]." order by id desc limit 10");
...
echo date_format($date,"d-m-Y");
											   ...
											   ?></td>
												 ...
												 <td> <button class="ppreview_d btn btn-sm btn-primary" data-id="<?php echo $row['id'];?>" data-bs-toggle="modal" data-bs-target="#myModal">Preview</button></td>
                                                <td>
												<?php 
												if($row['o_type'] == 'Self Pickup')
												{
												...
												<span  data-id="<?php echo $row['id'];?>" data-status="1" data-type="update_status" coll-type="porderapprove"  class=" drop btn btn-sm btn-success">Approve</span>
												<a href="?d_id=<?php echo $row['id']; ?>"  class="btn btn-sm btn-danger">Reject</a>
												<?php }else if($row['a_status'] == 1) {
													if($row['o_type'] == 'Self Pickup')
												{
													?>
													<span data-id="<?php echo $row['id'];?>" data-status="7" data-type="update_status" coll-type="pordercomplete"  class=" drop btn btn-sm btn-success">Complete Order</span>
													<?php 
												}
												else 
												{
