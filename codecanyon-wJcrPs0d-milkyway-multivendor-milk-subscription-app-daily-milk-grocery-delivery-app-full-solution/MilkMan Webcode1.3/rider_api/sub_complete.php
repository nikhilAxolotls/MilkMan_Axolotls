...
{
$check = $medi->query("select * from tbl_subscribe_order where id=".$id." and rid=".$rider_id."")->num_rows;
if($check != 0)
{
	$jpro = $medi->query("select * from tbl_subscribe_order_product where oid=".$id."");
$pops = array();
...
$table = "tbl_subscribe_order";
                ...
                $where =
                    "where id=" . $id . " and rid=".$rider_id."";
                $h = new Medico();
                $check = $h->mediupdateData($field, $table, $where);
				$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Subscription Order Completed Successfully!!!!!"); 
				
				$checks = $medi->query("select uid,store_id from tbl_subscribe_order where id=".$id."")->fetch_assoc(); 
				$uid = $checks['uid'];
...
$content = array(
       "en" => 'Subscription Order #'.$id.' Has Been Completed.'
   );
$heading = array(
   "en" => 'Subscription Order Completed!!'
);
...
$title_main = "Subscription Order Completed!!";
$description = 'Subscription Order #'.$id.' Has Been Completed.';
