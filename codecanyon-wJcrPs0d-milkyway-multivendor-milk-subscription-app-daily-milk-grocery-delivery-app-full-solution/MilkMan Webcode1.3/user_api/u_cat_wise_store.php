...
if ($data['uid'] == '' or $data['lats'] == '' or $data['longs'] == '' or $data['cat_id'] == '') {
    $returnArr = array(
        "ResponseCode" => "401",
        "Result" => "false",
...
while($rows = $sql_distances->fetch_assoc())
{
	...
	$querys = $medi->query("select * from service_details where  zone_id=".$rows['id']." and catid REGEXP  '\\\b".$cat_id."\\\b' and status=1  order by id desc");
...
$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Category Wise Store List Get Successfully!","CatWiseStoreData"=>$pops);
