...
class CartItemAdapter extends TypeAdapter<CartItem> {
  ...
  CartItem read(BinaryReader reader) {
    ...
    item.per = reader.readString(); // Product Discount
    item.isRequride = reader.readString(); // Subscription Required
    ...
    item.daysList = reader.readStringList(); // Days List
    item.cartCheck = reader.readString(); // Cart Check
    item.productID = reader.readString(); // Product ID
    item.storeSlogan = reader.readString(); // Store Slogan
    item.storeLogo = reader.readString(); // Store Logo
    return item;
  }

  @override
  void write(BinaryWriter writer, CartItem item) {
    writer.writeString(item.id ?? ""); // attribute ID
    writer.writeDouble(item.price ?? 0); // normal Price
    writer.writeInt(item.quantity ?? 0); // product Qty
    writer.writeDouble(item.productPrice ?? 0); // product Normal Price
    writer.writeString(item.strTitle ?? ""); // Store Title
    writer.writeString(item.per ?? ""); // Product Discount
    writer.writeString(item.isRequride ?? ""); // Subscription Required
    ...
    writer.writeStringList(item.daysList ?? []); // Days List
    writer.writeString(item.cartCheck ?? "0"); // Cart Check
    writer.writeString(item.productID ?? ""); // Product ID
    writer.writeString(item.storeSlogan ?? ""); // Store Slogan
    writer.writeString(item.storeLogo ?? ""); // Store Logo
  }
}
...
//     item.productID = reader.readString(); // Product Id
//     item.storeSlogan = reader.readString();
//     item.storeLogo = reader.readString();
//     return item;
//   }
...
//     writer.writeString(item.productID ?? ""); // Product Id
//     writer.writeString(item.storeLogo ?? "");
//   }
// }
